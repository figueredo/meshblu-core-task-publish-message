...
MessageWebhook = require '../src/publish-message'
...
describe 'MessageWebhook', ->
  ...
  describe '->do', ->
    context 'when given a valid sent message', ->
      beforeEach (done) ->
        @cache.subscribe 'sent:sender-uuid', (error) =>
          done error
    ...
    context 'when given a valid broadcast message', ->
      beforeEach (done) ->
        @cache.subscribe 'broadcast:sender-uuid', (error) =>
          done error
    ...
    context 'when given a valid received message', ->
      beforeEach (done) ->
        @cache.subscribe 'received:receiver-uuid', (error) =>
          done error
